...
| [docs/05-병렬-실험-계획.md](docs/05-병렬-실험-계획.md) | 타겟별 블로그 4개 병렬 실험 설계, 지표, 중단·집중 규칙 |

## 전제

- 플랫폼: 네이버 블로그 4개 병렬 실험 (`experiments/`), 12주 후 승자 집중
- 수익원: 애드포스트(타겟별 단가 비교) · 네이버 메이트 · 쇼핑커넥트
...
python scripts/weekly_plan.py --all --dry-run
python scripts/daily_draft.py --all --dry-run
